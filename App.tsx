...
const App: React.FC = () => {
  ...
  const refreshFromCloud = useCallback(async () => {
    ...
    try {
      ...
      setCloudError(`동기화 실패: ${err.message}`);
    ...
    <div className="min-h-screen flex flex-col md:flex-row bg-slate-50 overflow-hidden">
      {currentUser && (
        <button 
          ...
          className={`md:hidden fixed top-4 left-4 z-[40] bg-white text-indigo-600 p-3 rounded-2xl shadow-xl border border-indigo-50 transition-all ${isMenuOpen ? 'opacity-0 scale-90' : 'opacity-100 scale-100'}`}
        >
      ...
      {currentUser && (
        <>
          {isMenuOpen && (
            <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[50] md:hidden animate-in fade-in duration-300" onClick={() => setIsMenuOpen(false)} />
          )}

          <aside className={`fixed inset-y-0 left-0 z-[60] w-64 bg-indigo-700 text-white flex flex-col shadow-2xl transition-transform duration-500 md:static md:translate-x-0 md:h-screen md:w-60 ${isMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
            ...
            <div className="p-5 border-t border-indigo-600 bg-indigo-800/40">
              <div className="flex items-center space-x-3 mb-4">
                <div className="w-10 h-10 rounded-2xl bg-indigo-400 flex items-center justify-center font-black text-white">{sidebarName?.[0] || 'U'}</div>
                <div>
                  <p className="text-sm font-black truncate leading-tight">{sidebarName}</p>
                  <p className="text-[10px] text-indigo-300 font-bold uppercase tracking-wider">{roleLabel}</p>
                </div>
              </div>
              <button onClick={handleLogout} className="w-full py-2 bg-white/10 hover:bg-rose-500 rounded-xl text-[11px] font-black transition-all">로그아웃</button>
            </div>
          </aside>
        </>
      )}
      ...
      <main className="flex-1 overflow-y-auto p-5 md:p-10 max-w-7xl mx-auto w-full pt-20 md:pt-10">
        <Routes>
          ...
          <Route path="/sync" element={<DataManagement state={state} updateState={setState} cloudStatus={cloudStatus} cloudError={cloudError} onRefresh={refreshFromCloud} />} />
...
const CloudBadge = ({ status }: { status: 'OFFLINE' | 'CONNECTING' | 'LIVE' }) => (
  <div className={`flex items-center space-x-2 px-3 py-1.5 rounded-2xl text-[9px] font-black border ${status === 'LIVE' ? 'bg-emerald-500/10 text-emerald-300 border-emerald-400/20' : status === 'CONNECTING' ? 'bg-amber-500/10 text-amber-300 border-amber-400/20 animate-pulse' : 'bg-rose-500/10 text-rose-300 border-rose-400/20'}`}>
    <span className={`w-1.5 h-1.5 rounded-full ${status === 'LIVE' ? 'bg-emerald-400' : status === 'CONNECTING' ? 'bg-amber-400' : 'bg-rose-400'}`}></span>
    <span className="tracking-tight uppercase">{status === 'LIVE' ? 'Cloud Live' : status === 'CONNECTING' ? 'Syncing...' : 'Local Mode'}</span>
  </div>
);

const SidebarItem = ({ to, icon, label, active, onClick }: { to: string, icon: string, label: string, active: boolean, onClick: () => void }) => (
  <Link to={to} onClick={onClick} className={`flex items-center space-x-3 px-4 py-2 rounded-xl transition-all ${active ? 'bg-white text-indigo-700 shadow-xl font-black' : 'text-indigo-100 hover:bg-white/10 font-bold'}`}>
