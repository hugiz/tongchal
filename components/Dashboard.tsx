    ...
    <div className="space-y-10 pb-20 animate-in fade-in duration-500">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-slate-500 font-medium mt-1">{isDirector ? "학원 전체 실시간 지표입니다." : "담당 반의 오늘 현황입니다."}</p>
        </div>
        ...
        <div className="lg:col-span-12 space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {stats.visibleClasses.map(cls => {
              ...
              return (
                <div key={cls.id} className={`bg-white rounded-[40px] border transition-all duration-300 ${isSelected ? 'border-indigo-500 shadow-2xl ring-4 ring-indigo-50' : isClassToday ? 'border-amber-200 bg-amber-50/50' : 'border-slate-100 hover:shadow-xl'}`}>
                  <div className="p-7">
                    <div className="flex justify-between items-center mb-6">
                      <div className={`w-14 h-14 rounded-[22px] flex items-center justify-center text-2xl font-black ${isClassToday ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-200' : 'bg-slate-50 text-indigo-400'}`}>
                        {cls.name[0]}
                      </div>
                    ...
                    <div className="grid grid-cols-3 gap-3">
                      <button onClick={() => { setActiveActionClass(isSelected && activeTab === 'ATTENDANCE' ? null : cls.id); setActiveTab('ATTENDANCE'); }} className={`py-3.5 rounded-2xl text-[10px] font-black transition-all ${isSelected && activeTab === 'ATTENDANCE' ? 'bg-indigo-600 text-white shadow-md' : 'bg-slate-50 text-slate-600 hover:bg-slate-100'}`}>출석</button>
                      <button onClick={() => { setActiveActionClass(isSelected && activeTab === 'LEARNING' ? null : cls.id); setActiveTab('LEARNING'); }} className={`py-3.5 rounded-2xl text-[10px] font-black transition-all ${isSelected && activeTab === 'LEARNING' ? 'bg-emerald-600 text-white shadow-md' : 'bg-slate-50 text-slate-600 hover:bg-slate-100'}`}>학습</button>
                      <button onClick={() => { setActiveActionClass(isSelected && activeTab === 'CONSULTATION' ? null : cls.id); setActiveTab('CONSULTATION'); }} className={`py-3.5 rounded-2xl text-[10px] font-black transition-all ${isSelected && activeTab === 'CONSULTATION' ? 'bg-rose-600 text-white shadow-md' : 'bg-slate-50 text-slate-600 hover:bg-slate-100'}`}>상담</button>
                    </div>
                  </div>
                  {isSelected && (
                        ...
                        <div key={student.id} className="bg-white p-4 rounded-2xl border border-slate-100 flex items-center justify-between shadow-sm">
                          ...
                          <div className="flex gap-1 items-center">
                            ...
                            {activeTab === 'CONSULTATION' && (
                               <button onClick={() => navigate('/consultation')} className="text-[10px] font-black text-rose-500 bg-rose-50 px-4 py-2 rounded-xl border border-rose-100 hover:bg-rose-500 hover:text-white transition-all">일지</button>
                            )}
                          </div>
                        </div>
                      ))}
      ...
      <footer className="pt-20 pb-5 text-center">
        <div className="inline-flex items-center gap-3 px-6 py-2 bg-white rounded-3xl border border-slate-100 shadow-xl shadow-slate-200/50">
          ...
          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">EduLog System Ready • Last Sync: Today</p>
        </div>
      </footer>
    </div>
  );
