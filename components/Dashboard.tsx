...
const Dashboard: React.FC<DashboardProps> = ({ state, updateState, user }) => {
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      attendanceStatus: expectedCount > 0 ? `${presentCount} / ${expectedCount}` : "0 / 0",
    ...
    <div className="space-y-10 pb-20 animate-in fade-in duration-700">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
        <div className="lg:col-span-12 space-y-8">
          <div className="flex items-center justify-between">
            ...
            <p className="text-[10px] font-black text-slate-300 uppercase tracking-widest">Active System</p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {stats.visibleClasses.map(cls => {
              ...
              return (
                <div key={cls.id} className={`bg-white rounded-[40px] border transition-all duration-500 ${isSelected ? 'border-indigo-500 shadow-2xl ring-4 ring-indigo-50' : isClassToday ? 'border-amber-200 bg-amber-50/20' : 'border-slate-100 hover:shadow-xl'}`}>
                  <div className="p-7">
                    <div className="flex justify-between items-center mb-6">
                      <div className={`w-14 h-14 rounded-[20px] flex items-center justify-center text-2xl font-black ${isClassToday ? 'bg-indigo-600 text-white shadow-lg' : 'bg-slate-50 text-indigo-400'}`}>
                        {cls.name[0]}
                      </div>
                      {isClassToday && <span className="bg-amber-500 text-white text-[9px] font-black px-3 py-1 rounded-full animate-pulse">LIVE</span>}
                    </div>
                    <h4 className="font-black text-slate-800 text-xl mb-6">{cls.name}</h4>
                    <div className="grid grid-cols-3 gap-3">
                      <button onClick={() => { setActiveActionClass(isSelected && activeTab === 'ATTENDANCE' ? null : cls.id); setActiveTab('ATTENDANCE'); }} className={`py-3 rounded-2xl text-[10px] font-black transition-all ${isSelected && activeTab === 'ATTENDANCE' ? 'bg-indigo-600 text-white shadow-md' : 'bg-slate-50 text-slate-600'}`}>출석</button>
                      <button onClick={() => { setActiveActionClass(isSelected && activeTab === 'LEARNING' ? null : cls.id); setActiveTab('LEARNING'); }} className={`py-3 rounded-2xl text-[10px] font-black transition-all ${isSelected && activeTab === 'LEARNING' ? 'bg-emerald-600 text-white shadow-md' : 'bg-slate-50 text-slate-600'}`}>학습</button>
                      <button onClick={() => { setActiveActionClass(isSelected && activeTab === 'CONSULTATION' ? null : cls.id); setActiveTab('CONSULTATION'); }} className={`py-3 rounded-2xl text-[10px] font-black transition-all ${isSelected && activeTab === 'CONSULTATION' ? 'bg-rose-600 text-white shadow-md' : 'bg-slate-50 text-slate-600'}`}>상담</button>
                    </div>
                  </div>
                  {isSelected && (
                        ...
                        <div key={student.id} className="bg-white p-4 rounded-2xl border border-slate-100 flex items-center justify-between shadow-sm">
                          ...
                          <div className="flex gap-1">
                            {activeTab === 'ATTENDANCE' && ['PRESENT', 'LATE', 'ABSENT'].map(status => {
                              ...
                              return (
                                <button key={status} onClick={() => handleAttendance(student.id, cls.id, status as AttendanceStatus)} className={`px-2 py-1.5 rounded-xl text-[9px] font-black transition-all ${att?.status === status ? 'bg-indigo-600 text-white' : 'bg-slate-50 text-slate-400'}`}>
                                  {status === 'PRESENT' ? '출석' : status === 'LATE' ? '지각' : '결석'}
                                </button>
                              );
                            })}
                            {activeTab === 'LEARNING' && (
                               <input type="number" placeholder="P" className="w-12 px-2 py-1 rounded-lg border border-slate-200 text-xs font-black text-right outline-none focus:ring-2 focus:ring-emerald-500" onKeyDown={(e) => { if(e.key === 'Enter') { handleUpdateProgress(student.id, e.currentTarget.value); e.currentTarget.value = ''; alert('저장됨'); } }} />
                            )}
                            {activeTab === 'CONSULTATION' && (
                               <button onClick={() => navigate('/consultation')} className="text-[10px] font-black text-rose-500 bg-rose-50 px-3 py-1 rounded-xl border border-rose-100">일지</button>
                            )}
                          </div>
                        </div>
                      ))}
      ...
      <footer className="pt-20 pb-5 text-center">
        <div className="inline-flex items-center gap-3 px-6 py-2 bg-white rounded-3xl border border-slate-100 shadow-xl shadow-slate-200/50">
          ...
          <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">EduLog System Ready • Last Sync: Today</p>
        </div>
      </footer>
    </div>
  );
...
const StatCard = ({ title, value, icon, color }: { title: string, value: string | number, icon: string, color: string }) => (
  <div className="bg-white p-6 rounded-[32px] shadow-sm border border-slate-100 flex items-center space-x-4 transition-all hover:scale-105 active:scale-95 hover:shadow-xl hover:shadow-slate-200/40">
