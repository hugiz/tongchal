  ...
  const handleAttendance = (studentId: string, classId: string, status: AttendanceStatus) => {
    updateState(prev => {
      ...
      if (existingIdx > -1) {
        ...
        newAttendance.push({ id: `at_${Date.now()}_${Math.random().toString(36).substr(2, 5)}`, studentId, classId, date: today, status });
      }
    ...
    <div className="space-y-10 pb-20 animate-in fade-in duration-500">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-slate-500 font-medium mt-1">학원 실시간 운영 리포트입니다.</p>
        </div>
          ...
          return (
            <div key={cls.id} className={`bg-white rounded-[40px] border transition-all duration-300 ${isSelected ? 'border-indigo-500 shadow-2xl ring-4 ring-indigo-50' : isClassToday ? 'border-amber-200 bg-amber-50/30' : 'border-slate-100 hover:shadow-xl'}`}>
              <div className="p-7">
                ...
                <h4 className="font-black text-slate-800 text-xl mb-6 truncate">{cls.name}</h4>
                  ...
                  {classStudents.map(student => (
                    <div key={student.id} className="bg-white p-4 rounded-2xl border border-slate-100 flex items-center justify-between shadow-sm">
                      ...
                      <div className="flex gap-1 items-center">
                        ...
                        {activeTab === 'LEARNING' && (
                           <input type="number" placeholder="P" className="w-14 px-3 py-1.5 rounded-xl border border-slate-200 text-xs font-black text-right outline-none focus:ring-2 focus:ring-emerald-500" onKeyDown={(e) => { if(e.key === 'Enter') { handleUpdateProgress(student.id, e.currentTarget.value); e.currentTarget.value = ''; alert('기록되었습니다.'); } }} />
                        )}
      ...
      <footer className="pt-20 pb-5 text-center">
        <div className="sync-indicator">
          ...
          <span>EDU-LOG SYSTEM LIVE • V1.3.0 READY</span>
        </div>
      </footer>
    </div>
  );
